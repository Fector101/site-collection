...
import { parseDecimalSide } from './helper'
import { IMovieData } from "./api_data"


/**
 * Assume's rating starts from 0 to 5
 */
function ComputedStars({ rating }: { rating: number }) {
...
// { poster_path:string, overview:string, secs:number, title:string, vote_average:number } extends IMovieData
// interface ICard4Recommend extends IMovieData {
//     secs: number;
// }
function Card4Recommend({ data }: { data: IMovieData }) {
    const { poster_path, overview, secs = 7200, title, vote_average } = data
    return (
...
export default function Recommendations({ data }: { data: IMovieData[] }) {
    return (
